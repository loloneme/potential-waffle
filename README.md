...
## Создание команды
В API заметила противоречие - в описании эндпоинта создании команды написано, что он создает/обновляет пользователей, однако при попытке "создать" команду с тем же названием, должна возвращаться ошибка Team Already Exists. Я решила, что наиболее логичным решением будет реализвать Upsert пользователей команды. При запросе с названием команды, которая уже существет, пользователи, указанные в запросе, просто обновляются или создаются. Ошибка Team Already Exists не возвращается
